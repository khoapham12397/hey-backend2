...
// redisClient no dong vai tro interact to redis server :
// service of server start ngay tu dau roi do :

public class RedisCacheClient implements DataRepository {

    protected static final Logger LOGGER = LogManager.getLogger(RedisCacheClient.class);

    protected RedisClient client;

    protected static int numScanCount;
    ...
    // get from cache la duoc dung vay do :/// sau do the nao ??
    ...
    public Future<UserAuth> insertUserAuth(UserAuth userAuth) {
        ...
        // su dung cai thang nay no cung kha la tien loi dung la vya do :
        // sau do la the nao ??
        // dung la vay do :
        // cu goi la core concept of it ok chua dung vay do 
        ...
        client.hmset(generateUserAuthKey(userAuth.getUserName()), userAuthJsonObject, resInsertUserAuth -> {
            if (resInsertUserAuth.succeeded()) {
                ...
                // sinh 1 cai goi la wrapper proxy object -> .result() => object duoc cai object ben trong:
                // tu do 
            } else {
    ...
    // ca 2 cai deu la string het dun vay :
    ...
    public Future<ChatMessage> insertChatMessage(ChatMessage chatMessage) {
        ...
        chatMessageJsonObject.put("type", chatMessage.getMsgType());
        System.out.println("insert chat message with type = "+ chatMessage.getMsgType());
    ...
    public Future<ChatMessage> getChatMessage(String chatMessageKey) {
        Future<ChatMessage> future = Future.future();
        // tu cai key nay no re ra => get // hash 
                    ...
                    for (String fieldName : fieldNames) {
                        ...
                        switch (fieldName) {
                            ...
                            case "created_date":
                                ...
                                break;
                            case "type":
                            	chatMessage.setMsgType(Integer.parseInt(res.result().getString(fieldName)));
                            	break;
                            default:
    ...
    public Future<Long> deleteUnseenCount(String userId, String sessionId) {
        Future<Long> future = Future.future();
        // hieu van de nay roi ne: // khi ma thang kia no send 1 chat request den thi no can co 1 cai session id
        // no cai roomId dung la ya 
