...
public abstract class BaseService {
    ...
    // no return cai nay: // dung 
    public Future<Void> initTest(){
    	Future<Void> future = Future.future();
    	Future<JsonObject> futureData = initTestData();
    	futureData.setHandler(handler ->{
    		if(handler.succeeded()) {
    			...
    			future.fail(handler.cause());
    			// vi du the nay ne :dau tien la cia gi ??
    			// dunvya :
    		}
    	});
    	return future;
    ...
    public Future<JsonObject> initTestData() {
        ...
        createUsersFuture.compose(users -> {
            ...
            // friend ChatRoom session ID  id reing cua :
            // so sanh nguoc lai moi duoc dung cay:
            //
    ...
    private Future<List<UserStatus>> createUserStatuses(List<User> users) {
        ...
        // dung vay do ma just do it cai thang nay no la ay do :
        // dieu nay kha la tot dung roi don:
    ...
    // ChatList ??? la cai gi ? // dung la vay do:
    // dau tien nen xac dinh cu the lai no la cai gi???
    // dung la vay do ma : just do it sau do tien hanh hien thuc sau do di la duoc :
    // dun ay ;
